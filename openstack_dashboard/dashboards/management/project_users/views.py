...
from openstack_dashboard.dashboards.management.project_users \
    import tables as users_tables

from openstack_dashboard.dashboards.management.project_users \
    import forms as users_forms
...
class UpdateUserView(forms.ModalFormView):
    ...
    def get_object(self):
        try:
            return api.stacktask.user_get(self.request,
                                          self.kwargs['user_id'])
        except Exception:
...
class UsersView(tables.DataTableView):
    ...
    def get_data(self):
        try:
            return api.stacktask.user_list(self.request)
        except Exception:
