...
class ResendInvitation(tables.BatchAction):
    ...
    def action(self, request, datum_id):
        ...
        api.stacktask.user_invitation_resend(request, user.id)
...
class RevokeUser(tables.DeleteAction):
    help_text = _("This will remove the selected user(s) from the current project.")
...
class UpdateUserRow(tables.Row):
    ...
    def get_data(self, request, user_id):
        user = api.stacktask.user_get(request, user_id)
        return user
...
def UserRoleDisplayFilter(role_list):
    roles = [stacktask_util.get_role_text(r) for r in role_list]
    return ', '.join(roles)
